...
def file_draw_lund(fname, ptmin, ptmax, lundptmin=0, lundptmax=1e4, iterations=False):
	...
	#scond = "abs(j_jeta) < 2. && j_jpt > {} && j_jpt < {} && (j_lund_pt1 + j_lund_pt2) > 20.".format(ptmin, ptmax)
	#scond = "abs(j_jeta) < 2. && j_jpt > {} && j_jpt < {} && (j_lund_pt2) > 1.".format(ptmin, ptmax)
	...
	st = 'j_lund_logzdr:j_lund_log1odr>>hlund2D_tw({}, {}, {}, {}, {}, {})'.format(nbins, xmin, xmax, nbins, ymin, ymax)
	bwx = (xmax - xmin) / (nbins * 1.0)
	bwy = (ymax - ymin) / (nbins * 1.0)
	scond_lund_tw = '( ' + scond_lund + ') * (1./(j_lund_pt2 * j_lund_dR * j_lund_dR))'
	jt.Draw(st, scond_lund_tw, "e")
	hlund2D_tw = r.gDirectory.Get("hlund2D_tw")
	hlund2D_tw.GetXaxis().SetTitle("j_lund_log1odr")
	hlund2D_tw.GetYaxis().SetTitle("j_lund_logzdr")
	hlund2D_tw.Sumw2()
	# hlund2D_tw.Scale(1. / njets / (bwx * bwy))
	entries = hlund2D_tw.GetEntries()
	print entries, hlund2D_tw.Integral()
	# hlund2D_tw.Scale(1. / entries)
	hlund2D_tw_flat = hlund2D_tw.Clone("hlund2D_tw_flat")
	flatten_2D(hlund2D_tw_flat, 0.01)
	lpdg_list.append(hlund2D_tw)
	lpdg_list.append(hlund2D_tw_flat)

	st = 'j_lund_logzdr:j_lund_log1odr>>hlund2D_ntw({}, {}, {}, {}, {}, {})'.format(nbins, xmin, xmax, nbins, ymin, ymax)
	bwx = (xmax - xmin) / (nbins * 1.0)
	bwy = (ymax - ymin) / (nbins * 1.0)
	jt.Draw(st, scond_lund, "e")
	hlund2D_ntw = r.gDirectory.Get("hlund2D_ntw")
	hlund2D_ntw.GetXaxis().SetTitle("j_lund_log1odr")
	hlund2D_ntw.GetYaxis().SetTitle("j_lund_logzdr")
	hlund2D_ntw.Sumw2()
	# hlund2D_ntw.Scale(1. / njets / (bwx * bwy))
	hlund2D_ntw_flat = hlund2D_ntw.Clone("hlund2D_ntw_flat")
	flatten_2D(hlund2D_ntw_flat, 0.01)
	lpdg_list.append(hlund2D_ntw)
	lpdg_list.append(hlund2D_ntw_flat)
	...
	# this is for time not pTt
	for itdelta, tdelta in enumerate([[0, 1.e6], [0.0, 1.], [1., 10.], [10., 100.], [100., 1.e6], [0., 10.], [10., 100.], [10., 1.e6]]):
		hname = 'hlund2Dtime{}'.format(itdelta)
		st = 'j_lund_logzdr:j_lund_log1odr>>{}({}, {}, {}, {}, {}, {})'.format(hname, nbins, xmin, xmax, nbins, ymin, ymax)
		bwx = (xmax - xmin) / (nbins * 1.0)
		bwy = (ymax - ymin) / (nbins * 1.0)
		scond_tf0 = " && (1./(j_lund_pt2*j_lund_dR*j_lund_dR) > {}) && (1./(j_lund_pt2*j_lund_dR*j_lund_dR) < {})".format(tdelta[0], tdelta[1])
		jt.Draw(st, scond_lund + scond_tf0, "e")
		hlund2Dtime = r.gDirectory.Get(hname)
		hlund2Dtime.GetXaxis().SetTitle("j_lund_log1odr")
		hlund2Dtime.GetYaxis().SetTitle("j_lund_logzdr")
		hlund2Dtime.Sumw2()
		hlund2Dtime.Scale(1. / njets / (bwx * bwy))
		hlund2Dtime_flat = hlund2Dtime.Clone(hname + "_flat")
		flatten_2D(hlund2Dtime_flat)

		hit.append(hlund2Dtime)
		hit.append(hlund2Dtime_flat)

		hname = 'hlund2Dtime{}_z'.format(itdelta)
		st = 'j_lund_z>>{}({}, {}, {})'.format(hname, nbins_z, 0, 0.5)
		scond_tf0_z = scond_tf0
		jt.Draw(st, scond_lund + scond_tf0_z, "e")
		hlund2Dtime_z = r.gDirectory.Get(hname)
		hlund2Dtime_z.GetXaxis().SetTitle("z")
		hlund2Dtime_z.Scale(1. / njets / (bwx_z))

		hit.append(hlund2Dtime_z)


	# useful profiles
	proft = r.TProfile("proft", "proft;p_{T,a}+p_{T,b};#LT t #GT = #LT 1/(k_{T} #Delta) #GT = #LT 1/(p_{T,b} #Delta^{2}) #GT", 10, 0, ptmax)
	jt.Draw("1/(j_lund_dR*j_lund_dR*j_lund_pt1):(j_lund_pt1 + j_lund_pt2)>>proft", "abs(j_jeta) < 2. && j_jpt > {} && j_jpt < {}".format(ptmin, ptmax), "colz")
	hit.append(proft)

	proft2 = r.TProfile("proft2", "proft2;ln(1/#Delta);#LT t #GT = #LT 1/(k_{T} #Delta) #GT = #LT 1/(p_{T,b} #Delta^{2}) #GT", 10*3, 0.9, 6)
	jt.Draw("1/(j_lund_dR*j_lund_dR*j_lund_pt2):j_lund_log1odr>>proft2", "abs(j_jeta) < 2. && j_jpt > {} && j_jpt < {}".format(ptmin, ptmax), "colz")
	hit.append(proft2)

	proft3 = r.TProfile("proft3", "proft3;n_{split};#LT t #GT = #LT 1/(k_{T} #Delta) #GT = #LT 1/(p_{T,b} #Delta^{2}) #GT", 11, 0, 11)
	jt.Draw("1/(j_lund_dR*j_lund_dR*j_lund_pt2):Iteration$>>proft3", "abs(j_jeta) < 2. && j_jpt > {} && j_jpt < {}".format(ptmin, ptmax), "colz")
	hit.append(proft3)

	proft4 = r.TProfile("proft4", "proft4;z;#LT t #GT = #LT 1/(k_{T} #Delta) #GT = #LT 1/(p_{T,b} #Delta^{2}) #GT", 10, 0, 0.5)
	jt.Draw("1/(j_lund_dR*j_lund_dR*j_lund_pt2):j_lund_z>>proft4", "abs(j_jeta) < 2. && j_jpt > {} && j_jpt < {}".format(ptmin, ptmax), "colz")
	hit.append(proft4)

	profdR = r.TProfile("profdR", "profdR;n_{iter};#LT #Delta #GT", 11, 0, 11);
	jt.Draw("(j_lund_dR):Iteration$>>profdR", "abs(j_jeta) < 2. && j_jpt > {} && j_jpt < {}".format(ptmin, ptmax), "colz");
	hit.append(profdR)

	profpt = r.TProfile("profpt", "profpt;n_{iter};#LT p_{T,a} + p_{T,b} #GT", 20, ptmin, ptmax)
	jt.Draw("(j_lund_pt1 + j_lund_pt2):Iteration$>>profpt", "abs(j_jeta) < 2. && j_jpt > {} && j_jpt < {}".format(ptmin, ptmax), "colz");
	hit.append(profpt)
	...
	#foutname = foutname.replace('.root', '_ptemit20.root')
	#foutname = foutname.replace('.root', '_ptb_1.root')
